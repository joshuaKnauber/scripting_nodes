...
class SN_PT_ExtensionsPanel(bpy.types.Panel):
    ...
    def draw_header(self, context):
        ...
        layout.operator("wm.url_open", text="", icon="QUESTION",
                        emboss=False).url = "https://joshuaknauber.notion.site/Packages-Snippets-5fc9492b640146a2bcafb269d4a9e876"
...
class SN_PT_SnippetsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.scale_y = 1.1
        row.operator("sn.open_preferences", text="Get Snippets",
                     icon="URL").navigation = "MARKET"
        ...
        if node and node.select and node.bl_idname in ["SN_RunFunctionNode", "SN_RunInterfaceFunctionNode"]:
            if getattr(node, "ref_SN_FunctionNode", None) or getattr(node, "ref_SN_InterfaceFunctionNode", None):
                op = row.operator(
                    "sn.draw_export_snippet", text="Export Snippet", icon="EXPORT", depress=True)
                op.node = node.name
                ...
                box = row.box()
                box.label(
                    text="Select a valid Run Function node to export a snippet", icon="EXPORT")
        else:
            box = row.box()
            box.label(
        ...
        row = layout.row()
        row.scale_y = 1.1
        row.operator("sn.install_snippet",
                     text="Install Snippets", icon="FILE_FOLDER")
        ...
        for i, snippet in enumerate(snippet_ops.loaded_snippets):
            ...
            if type(snippet) == str:
                ...
                row.operator("sn.uninstall_snippet", text="",
                             icon="PANEL_CLOSE", emboss=False).index = i
            else:
                cat = context.scene.sn.snippet_categories.get(
                    snippet["name"])
                row.prop(cat, "expand", text="", emboss=False,
                         icon="DISCLOSURE_TRI_DOWN" if cat.expand else "DISCLOSURE_TRI_RIGHT")
...
class SN_PT_PackagesPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.scale_y = 1.1
        row.operator("sn.open_preferences", text="Get Packages",
                     icon="URL").navigation = "MARKET"
        layout.separator()
        ...
        row.operator("sn.install_package",
                     text="Install Package", icon="FILE_FOLDER")
        ...
        for i, package in enumerate(package_ops.loaded_packages):
            ...
            if package["wiki"]:
                row.operator("wm.url_open", text="", icon="URL",
                             emboss=False).url = package["wiki"]
            row.operator("sn.uninstall_package", text="",
                         icon="PANEL_CLOSE", emboss=False).index = i
