...
def get_monospace_font() -> int:
    ...
    if not _monospace_font_id:
        font_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), "assets", "fonts", "DejaVuSansMono.woff2")
...
def draw_error_marker(node: bpy.types.Node):
    ...
    # border
    shader.uniform_float("color", (.35, 0, .05, 1))
    draw_filled_circle((x, y), size/2, 16, shader)
    # circle
    shader.uniform_float("color", (.7, 0, .15, 1))
