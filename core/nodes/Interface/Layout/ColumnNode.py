...
class SNA_NodeColumn(SNA_BaseNode, bpy.types.Node):
    bl_idname = "SNA_NodeColumn"
    bl_label = "Column"
    ...
    def generate(self, context, trigger):
        ...
        self.code = f"""
            col_{self.id} = {layout}.column({props})
            {self.inputs["Decorate"].editable and f"col_{self.id}.use_property_decorate = {self.inputs['Decorate'].get_code()}" or ""}
            {self.inputs["Split Layout"].editable and f"col_{self.id}.use_property_split = {self.inputs['Split Layout'].get_code()}" or ""}
            {self.inputs["Active"].editable and f"col_{self.id}.active = {self.inputs['Active'].get_code()}" or ""}
            {self.inputs["Enabled"].editable and f"col_{self.id}.enabled = {self.inputs['Enabled'].get_code()}" or ""}
            {self.inputs["Alert"].editable and f"col_{self.id}.alert = {self.inputs['Alert'].get_code()}" or ""}
            {self.inputs["Scale"].editable and f"col_{self.id}.scale_x, col_{self.id}.scale_y = {self.inputs['Scale'].get_code()}" or ""}
            {self.inputs["Alignment"].editable and f"col_{self.id}.alignment = {self.inputs['Alignment'].get_code()}" or ""}
            {self.inputs["Context"].editable and f"col_{self.id}.operator_context = {self.inputs['Context'].get_code()}" or ""}
            {self.outputs["Interface"].get_code(3)}
        """

        self.outputs["Interface"].set_meta("layout", f"col_{self.id}")
