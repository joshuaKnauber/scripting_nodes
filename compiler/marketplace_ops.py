...
class SN_OT_CopyCommand(bpy.types.Operator):
    ...
    def execute(self, context):
        tree = context.scene.sn.addon_tree()

        category = tree.sn_graphs[0].category
        if category == "CUSTOM":
            category = tree.sn_graphs[0].custom_category
            
        addon_info = {
            "name": tree.sn_graphs[0].name,
            "description": tree.sn_graphs[0].description,
            "category": category,
            "author": tree.sn_graphs[0].author,
            "blender_version": list(tree.sn_graphs[0].blender),
            "addon_version": list(tree.sn_graphs[0].version),
            "external": self.url != "",
            "url": self.url,
            "price": self.price,
            "blend": self.blender,
...
class SN_OT_ExportToMarketplaceAddon(bpy.types.Operator):
    ...
    def draw(self,context):
        box = self.layout.box()
        box.label(text="If you think your addon is interesting to others you can upload it here!",icon="QUESTION")
