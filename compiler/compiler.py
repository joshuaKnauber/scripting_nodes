...
def compile_addon(addon_tree, is_export=False):
    ...
    try:
        ...
        # add addon info
        addon_data["code"]["addon_info"] = normalize_code(
            create_addon_info(addon_tree), 0)
        ...
        for graph in addon_tree.sn_graphs:
            if not graph.node_tree.has_changes and graph.name in addon_data["code"]["graph_code"]:
                addon_did_once = {
                    **addon_did_once, **addon_data["code"]["graph_code"][graph.name]["did_once"]}
        ...
        for graph in addon_tree.sn_graphs:
            if graph.node_tree.has_changes or not graph.name in addon_data["code"]["graph_code"] or graph.name in error_graphs:
                ...
                # make graph code
                graph_code, graph_did_once = __evaluate_graph(
                    graph, addon_tree, addon_did_once)
        ...
        # write easy bpy
        if bpy.context.scene.sn.easy_bpy and len(bpy.context.scene.sn.easy_bpy.lines):
            if is_export:
                __write_in_text(addon_data["text"],
                                "from .easybpy import *")
            else:
                ...
                __write_in_text(
                    addon_data["text"], "sys.modules['easybpy_serpens'] = bpy.data.texts['easybpy.py'].as_module()")
                __write_in_text(addon_data["text"],
                                "from easybpy_serpens import *")

        # write variable define block
        __write_blockcomment(addon_data["text"], "INITALIZE VARIABLES")
        __write_in_text(addon_data["text"], normalize_code(
            __create_variable_register(addon_tree), 0))
        ...
        for graph in addon_data["code"]["graph_code"]:
            if addon_data["code"]["graph_code"][graph]["imperative"]:
                ...
                __write_in_text(
                    addon_data["text"], addon_data["code"]["graph_code"][graph]["imperative"])
        ...
        for graph in addon_data["code"]["graph_code"]:
            if addon_data["code"]["graph_code"][graph]["evaluated"]:
                __write_graphcomment(addon_data["text"], graph)
                __write_in_text(
                    addon_data["text"], addon_data["code"]["graph_code"][graph]["evaluated"])

        # write register function
        __write_blockcomment(addon_data["text"], "REGISTER ICONS")
        __write_in_text(addon_data["text"], normalize_code(
            __create_icon_register(addon_tree), 0))

        __write_blockcomment(addon_data["text"], "REGISTER PROPERTIES")
        __write_in_text(addon_data["text"], normalize_code(
            __create_property_register(addon_tree), 0))
        __write_paragraphs(addon_data["text"], 1)
        __write_in_text(addon_data["text"], normalize_code(
            __create_property_unregister(addon_tree), 0))
        ...
        unregister_code = __get_unregister_code(
            addon_data["code"]["graph_code"])
        ...
        if not is_export:
            ...
            __write_in_text(addon_data["text"], "   import sys")
            __write_in_text(
                addon_data["text"], "   sys.modules['blender_visual_scripting_addon'].compiler.compiler.unreg_func = unregister")
        ...
        # save time
        end_time = time()
        addon_tree.sn_graphs[0].last_compile_time = str(
            round(end_time-start_time, 4))+"s"
...
def __evaluate_graph(graph, addon_tree, addon_did_once):
    graph_code = {"imports": "", "imperative": "",
                  "evaluated": "", "register": [], "unregister": []}
    graph_did_once = {"imports": [], "imperative": [],
                      "evaluated": [], "register": [], "unregister": []}
    ...
    for node in graph.node_tree.nodes:
        if not node.bl_idname in ["NodeFrame", "NodeReroute"]:
            ...
            if __should_import(node, {**addon_did_once, **graph_did_once}["imports"]):
                graph_code["imports"] += process_returned(
                    node, node.code_imports(bpy.context))
                if not node.bl_idname in graph_did_once["imports"]:
                    graph_did_once["imports"].append(node.bl_idname)

            if __should_imperative(node, {**addon_did_once, **graph_did_once}["imperative"]):
                graph_code["imperative"] += process_returned(
                    node, node.code_imperative(bpy.context))
                if not node.bl_idname in graph_did_once["imperative"]:
                    graph_did_once["imperative"].append(node.bl_idname)

            if __should_evaluate(node, {**addon_did_once, **graph_did_once}["evaluated"]):
                graph_code["evaluated"] += process_node(node, None)
                if not node.bl_idname in graph_did_once["evaluated"]:
                    graph_did_once["evaluated"].append(node.bl_idname)
            ...
            if __should_register(node, {**addon_did_once, **graph_did_once}["register"]):
                ...
                graph_code["register"].append(
                    (process_returned(node, node.code_register(bpy.context)), order))
                if not node.bl_idname in graph_did_once["register"]:
                    graph_did_once["register"].append(node.bl_idname)

            if __should_unregister(node, {**addon_did_once, **graph_did_once}["unregister"]):
                order = 0
                if "register_order" in node.node_options:
                    order = node.node_options["register_order"]
                graph_code["unregister"].append(
                    (process_returned(node, node.code_unregister(bpy.context)), order))
                if not node.bl_idname in graph_did_once["unregister"]:
                    graph_did_once["unregister"].append(node.bl_idname)
