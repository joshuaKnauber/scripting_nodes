...
class SN_PathInfoNode(bpy.types.Node, SN_ScriptingBaseNode):
    ...
    def evaluate(self, context):
        ...
        self.outputs['Directory Name'].python_value = f"os.path.dirname({self.inputs['Path'].python_value})"
        self.outputs['File Name'].python_value = f"os.path.basename({self.inputs['Path'].python_value})"
