...
class SN_VectorInputNode(bpy.types.Node, SN_ScriptingBaseNode):
    ...
    docs = {
        "text": ["The display vector node can <important>draw a vector</> in your interface.",
                "",
                "Internal/Custom: <subtext>Internal uses the Input Socket while custom is looking for Vector variables in your nodes</>",
        ...
        if self.search_prop == "internal":
            ...
            if self.search_value in bpy.context.space_data.node_tree.search_variables:
                if bpy.context.space_data.node_tree.search_variables[self.search_value].type == "vector" or bpy.context.space_data.node_tree.search_variables[self.search_value].type == "four_vector":
                    ...
                    box = layout.box()
                    box.label(text="Please select a vector variable")
    ...
    def evaluate(self, socket, node_data, errors):
        ...
        if self.search_prop == "internal":
            ...
            if self.search_value in node_data["node_tree"].search_variables:
                if node_data["node_tree"].search_variables[self.search_value].type in ["vector", "four_vector"]:
                    if len(self.inputs) == 4:
                    ...
                    errors.append({"title": "Wrong variable selected", "message": "You need to select a vector variable", "node": self, "fatal": True})
                    return {"blocks": [{"lines": [],"indented": []}],"errors": errors}
