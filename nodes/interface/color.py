...
class SN_ColorInputNode(bpy.types.Node, SN_ScriptingBaseNode):
    ...
    docs = {
        "text": ["The display color node can <important>draw a color selection</> in your interface.",
                "",
                "Internal/Custom: <subtext>Internal uses the Input Socket while custom is looking for Color variables in your nodes</>",
                ...
                "Input: <subtext>The object data that contains the vector property</>"],
        "python": ["layout.<function>prop</>(data, <string>\"vector_property\"</>, text=<string>\"My input text\"</>)"]
    }
    ...
    def update_name(self, context):
        if self.search_prop == "custom":
            if self.search_value in bpy.context.space_data.node_tree.search_variables:
                if bpy.context.space_data.node_tree.search_variables[self.search_value].type == "color" or bpy.context.space_data.node_tree.search_variables[self.search_value].type == "four_color":
        ...
        if self.search_prop == "internal":
            ...
            if self.search_value in bpy.context.space_data.node_tree.search_variables:
                if bpy.context.space_data.node_tree.search_variables[self.search_value].type == "color" or bpy.context.space_data.node_tree.search_variables[self.search_value].type == "four_color":
                    ...
                    box = layout.box()
                    box.label(text="Please select a color variable")
    ...
    def evaluate(self, socket, node_data, errors):
        ...
        if self.search_prop == "internal":
            ...
            if self.search_value in node_data["node_tree"].search_variables:
                if node_data["node_tree"].search_variables[self.search_value].type in ["color", "four_color"]:
                    if len(self.inputs) == 4:
                    ...
                    errors.append({"title": "Wrong variable selected", "message": "You need to select a color variable", "node": self, "fatal": True})
                    return {"blocks": [{"lines": [],"indented": []}],"errors": errors}
