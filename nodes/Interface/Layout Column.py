...
class SN_LayoutColumnNodeNew(bpy.types.Node, SN_ScriptingBaseNode):
    ...
    def evaluate(self, context):
        self.code = f"""
                    ...
                    col_{self.static_uid}.operator_context = "INVOKE_DEFAULT" if {"False" if not "Use Invoke" in self.inputs else self.inputs["Use Invoke"].python_value} else "EXEC_DEFAULT"
