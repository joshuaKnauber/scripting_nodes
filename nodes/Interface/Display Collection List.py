...
class SN_DisplayCollectionListNode(bpy.types.Node, SN_ScriptingBaseNode):
    ...
    def on_create(self, context):
        ...
        self.add_property_input("Collection")
    ...
    def evaluate(self, context):
        if self.inputs["Index Property"].is_linked and self.inputs["Collection"].is_linked:
            ...
            self.code_unregister = f"""
                                    ...
                                    """
            self.code = f"{self.active_layout}.template_list('{ui_list_idname}', '{self.name}', {self.inputs['Collection'].python_value_source}, '{self.inputs['Collection'].python_value_name}', {self.inputs['Index Property'].python_value_source}, '{self.inputs['Index Property'].python_value_name}', rows={self.inputs['Rows'].python_value})"
            self.outputs["Item"].python_value = f"{self.inputs['Collection'].python_value}[index]"
        else:
