...
class SNA_OT_StringPropertySettings(bpy.types.Operator):
    """Configure string property settings"""

    bl_idname = "sna.string_property_settings"
    bl_label = "String Property Settings"
    bl_description = "Configure string property options"
    ...
    def draw(self, context):
        ...
        # Subtype and max length
        layout.prop(node, "prop_subtype")
        layout.prop(node, "prop_maxlen")
...
class SNA_Node_StringProperty(ScriptingBaseNode, bpy.types.Node):
    ...
    def prop_name(self):
        ...
        # Generate name from label: lowercase, replace spaces with underscores, add id suffix
        import re
    ...
    prop_maxlen: bpy.props.IntProperty(
        name="Max Length",
        description="Maximum length of the string (0 for unlimited)",
        default=0,
        min=0,
        update=update_props,
    )
    ...
    prop_subtype: bpy.props.EnumProperty(
        ...
        name="Subtype",
        description="Subtype of the string property",
        default="NONE",
        update=update_props,
    )
    ...
    def draw(self, context, layout):
        ...
        # Settings button
        op = layout.operator(
            "sna.string_property_settings", text="Settings", icon="PREFERENCES"
        )
        op.node_id = self.id
    ...
    def _build_prop_args(self):
        ...
        if self.prop_maxlen > 0:
            prop_args.append(f"maxlen={self.prop_maxlen}")
    ...
    def class_body_annotation(self):
        ...
        return f"{self.prop_name}: bpy.props.StringProperty({', '.join(prop_args)})"
...
bpy.types.{self.register_on}.{self.prop_name} = bpy.props.StringProperty(
    {args_str}
)
"""
