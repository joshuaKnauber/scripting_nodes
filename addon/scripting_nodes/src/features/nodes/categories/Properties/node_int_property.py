...
class SNA_OT_IntPropertySettings(bpy.types.Operator):
    """Configure integer property settings"""

    bl_idname = "sna.int_property_settings"
    bl_label = "Integer Property Settings"
    bl_description = "Configure integer property options"
...
class SNA_Node_IntProperty(ScriptingBaseNode, bpy.types.Node):
    ...
    prop_default: bpy.props.IntProperty(
        name="Default",
    ...
    prop_min: bpy.props.IntProperty(
        name="Min",
    ...
    prop_max: bpy.props.IntProperty(
        name="Max",
    ...
    prop_soft_min: bpy.props.IntProperty(
        name="Soft Min",
    ...
    prop_soft_max: bpy.props.IntProperty(
        name="Soft Max",
    ...
    prop_step: bpy.props.IntProperty(
        name="Step",
        description="Step of increment/decrement in UI",
        default=1,
        min=1,
        max=100,
        update=update_props,
    ...
    prop_subtype: bpy.props.EnumProperty(
        ...
        name="Subtype",
        description="Subtype of the integer property",
        default="NONE",
        update=update_props,
    )
    ...
    def draw(self, context, layout):
        ...
        # Settings button
        op = layout.operator(
            "sna.int_property_settings", text="Settings", icon="PREFERENCES"
        )
        op.node_id = self.id

    def _build_prop_args(self):
        """Build the argument list passed to bpy.props.IntProperty(...).

        Shared between runtime registration (code_register) and class-body
        annotation - same args either way.
        """
        options = []
        ...
        if self.prop_min > -2147483648:
            prop_args.append(f"min={self.prop_min}")
        if self.prop_max < 2147483647:
    ...
    def class_body_annotation(self):
        """Single-line `name: bpy.props.IntProperty(...)` for class-body use."""
        prop_args, _, _ = self._build_prop_args()
        return f"{self.prop_name}: bpy.props.IntProperty({', '.join(prop_args)})"

    def generate(self):
        prop_args, has_update, update_socket = self._build_prop_args()
        args_str = ",\n        ".join(prop_args)

        # Update callback stays module-scope whether runtime- or class-body-
...
"""
        ...
        # Class-body targets are emitted inline by the container node, so we
        # don't produce register/unregister code here
        if self.is_class_body_target:
            return

        self.code_register = f"""
bpy.types.{self.register_on}.{self.prop_name} = bpy.props.IntProperty(
    {args_str}
)
"""
