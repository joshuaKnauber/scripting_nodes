...
The `register_on` enum on each property node now includes Operator and
Preferences. When one of those is selected the node emits no
code_register/unregister; instead it exposes a `class_body_annotation()`
method that the consuming container node uses to inject the property into
its emitted class.
"""
