...
class SNA_Node_BlendData(ScriptingBaseNode, bpy.types.Node):
    ...
    def draw(self, context, layout):
        ...
        if not self.data_path:
            layout.operator("sna.setup_blend_data_node", text="Setup from Clipboard", icon="PASTEDOWN").node_name = self.name
        else:
            # Show access mode controls if this is a collection accessor
            if self.access_mode != "NONE":
                layout.prop(self, "access_mode", text="")
    
    def setup_from_path(self, path: str, is_root: bool = True, access_mode: str = "NONE", output_type: str = "ScriptingBlendDataSocket", input_name: str = "Data"):
