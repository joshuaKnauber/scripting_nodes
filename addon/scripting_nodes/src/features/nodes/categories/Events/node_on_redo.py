...
class SNA_Node_OnRedo(ScriptingBaseNode, bpy.types.Node):
    bl_idname = "SNA_Node_OnRedo"
    bl_label = "On Redo"
    sn_options = {"ROOT_NODE"}

    action: bpy.props.EnumProperty(
        items=[
            ("redo_pre", "Before", "On loading a redo step (before)"),
            ("redo_post", "After", "On loading a redo step (after)"),
        ],
        name="Time of Action",
