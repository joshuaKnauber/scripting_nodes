...
class SNA_Node_OnUndo(ScriptingBaseNode, bpy.types.Node):
    bl_idname = "SNA_Node_OnUndo"
    bl_label = "On Undo"
    sn_options = {"ROOT_NODE"}

    action: bpy.props.EnumProperty(
        items=[
            ("undo_pre", "Before", "On loading an undo step (before)"),
            ("undo_post", "After", "On loading an undo step (after)"),
        ],
        name="Time of Action",
