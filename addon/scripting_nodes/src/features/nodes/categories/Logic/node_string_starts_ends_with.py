    ...
    def generate(self):
        ...
        if self.detection_mode == "STARTSWITH":
            self.outputs[0].code = (
                ...
                f"({self.inputs[1].eval()}.startswith({self.inputs[2].eval()}) " 
                f"if {self.inputs[0].eval()} else {self.inputs[1].eval()}.lower().startswith({self.inputs[2].eval()}.lower())))"
            )
        else:
            self.outputs[0].code = (
                f"(False if not {self.inputs[3].eval()} else "
                f"({self.inputs[1].eval()}.endswith({self.inputs[3].eval()}) " 
                f"if {self.inputs[0].eval()} else {self.inputs[1].eval()}.lower().endswith({self.inputs[3].eval()}.lower())))"
            )
