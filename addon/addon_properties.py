...
class SN_AddonProperties(bpy.types.PropertyGroup):
    ...
    def _execute_node(self, node_tree_id: str, node_id: str, locals: dict, globals: dict):
    ...
    def update_active_nodetree_index(self, context: bpy.types.Context):
        try:
            context.space_data.node_tree = bpy.data.node_groups[self.active_nodetree_index]
        except IndexError:
            pass

    active_nodetree_index: bpy.props.IntProperty(default=0, name="Active Node Tree Index", description="The index of the active node tree", min=0, update=update_active_nodetree_index)
    ...
    property_type: bpy.props.EnumProperty(items=property_type_items, name="Property Type", description="The type of property to display")
